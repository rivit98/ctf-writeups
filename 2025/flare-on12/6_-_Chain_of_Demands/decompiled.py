...
if __name__ == '__main__':
    logic = ChatLogic()
    seed_hash = logic._get_system_artifact_hash()
    m, c, n = logic._generate_primes_from_hash(seed_hash)
    lcg_oracle = LCGOracle(m, c, n, seed_hash)

    print(f'Seed Hash: {seed_hash}')
    print(f'LCG Multiplier (m): {m}')
    print(f'LCG Increment (c): {c}')
    print(f'LCG Modulus (n): {n}')
    print('\nGenerating first 5 LCG outputs:')
    for i in range(6):
        val = lcg_oracle.get_next(i)
