...
def main():
    ...
    for n1 in nodes:
        for n2 in nodes:
            ...
            if n2 == nodes[-1]:
                # if we are returning to the capital, set weight to zero
                G.add_edge(n1, n2, weight=0)
            else:
                G.add_edge(n1, n2, weight=calcDistance(t[n1], t[n2]))
...
if __name__ == "__main__":
    ...
    print(coordinates)
    print(
        f"If we send an owl to visit the cities in this order, it will fly a total of {calcPath([capital] + towns)} kilometers.")
    print("We believe that the best path exist.")
    user_input = input(
        "Please enter best permutation of this towns list.\n*just numbers, space-separated for example: 0 1 2 3 4 ... 198\n")
...
# ctfzone{7r4v3ll1n6_54l35m4n_h4v3_n0_pr0bl3m_47_4ll_w17h_y0u}